...
void main() {
  group('Testing TxtLoader', () {
    ...
    bookConfig.updateSize(300, 300);
    TxtLoader txtLoader = TxtLoader(book, bookConfig);
    // test('match toc test', testMatchToc(txtLoader),);
    test(
      'match test',
      () async {
        RegExp regExp = RegExp(r'[\ \r\n]+');
        regExp.allMatches("   第一章 - 居然会赢 ").forEach((element) {
          printD("allMatches", "element: ${element.group(0)}");
        });
        String a = "   第一章 - 居然会赢 ".replaceAll(RegExp(r'[\s]+'), "!");
        printD("【$a】");
      },
    );
  });
...
testMatchToc(TxtLoader txtLoader) async {
  ...
  expect(
      matchTitle(txtLoader, "外传 第一万二千三百四十五章 - 居然会赢"), "外传 第一万二千三百四十五章 - 居然会赢");

...
class OperatorWidget extends StatefulWidget {
  const OperatorWidget(this._controller, {super.key});
...
class OperatorWidgetState extends State<OperatorWidget> {
  ...
  late MapEntry<String, Function> toggleMenu =
      MapEntry("菜单", context.read<BookReadViewModel>().showMenu);
  ...
  void _nextPage() {
    int max = context.read<BookReadViewModel>().showPages.length;
...
class _TouchBlockView extends StatelessWidget {
  const _TouchBlockView(this._map, {this.color = Colors.transparent});
